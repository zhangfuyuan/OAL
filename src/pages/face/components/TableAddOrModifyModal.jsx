...
const TableAddOrModifyModal = props => {
  const { form, bean, visible, handleSubmit, handleCancel, dispatch, groupId, supportIcCard } = props;
  ...
  const handleOk = () => {
    form.validateFields((err, fieldsValue) => {
      if (err) return;
      const { name, staffid, icCard, userPhotos } = fieldsValue;
      ...
      if (_userPhotosLen > 0) {
        ...
        if (window.WebUploader) {
          ...
          const { faceId, name, staffid, icCard, groupId, isEdit, peopleType, isUpdateImg } = params || {};
        ...
        setUploadLoading(true);
        dispatch({
          type: isEdit ? 'face/editInfo' : 'face/addInfo',
          payload: params,
        }).then(res => {
          if (!visible) return;
  ...
  const wuInit = (newFileList) => {
    ...
    newFileList.forEach((item, index) => {
      ...
      const { faceId, staffname, staffid, icCard, groupId, isEdit, peopleType, isUpdateImg } = item || {};
  ...
  const wuUpload = () => {
    ...
    uploader.on('uploadBeforeSend', (block, data) => {
      if (!visible) return;
      const { file: { md5, faceId, staffname, staffid, icCard, groupId, isEdit, peopleType, isUpdateImg } } = block || { file: {} };
      ...
      <Form {...formItemLayout}>
        ...
        <Form.Item label={formatMessage({ id: 'oal.face.staffid' })}>
          {getFieldDecorator('staffid', {
            ...
            initialValue: bean && bean.staffid || '',
          })(<Input placeholder={formatMessage({ id: 'oal.face.enterStaffid' })} disabled={!!bean && !!bean.staffid} />)}
        </Form.Item>
        {
          supportIcCard ?
            (
