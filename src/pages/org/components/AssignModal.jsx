...
const AssignModal = props => {
  ...
  const checkPoints = (rule, value, callback) => {
    if (value && value.replace) {
      ...
      if (_val < beanTerminalAssigned) {
        callback(formatMessage({ id: 'oal.org.totalNotLessAssign' }));
  ...
  return (
    <Modal
      destroyOnClose
      title={`${formatMessage({ id: 'oal.common.assignTo' })} ${bean && bean.name || '-'}`}
      visible={visible}
      onOk={handleOk}
    ...
    >
      <div style={{ textAlign: 'center', marginBottom: '20px' }}>
        ...
        <span>
          <span style={{ color: 'rgba(0, 0, 0, 0.85)' }}>
            <FormattedMessage id="oal.org.orgPoints" /> :
                </span>
      ...
      <Form {...formItemLayout}>
        <Form.Item label={formatMessage({ id: 'oal.org.orgTotalPoints' })}>
          ...
          })(<Input autoFocus placeholder={formatMessage({ id: 'oal.org.orgTotalPoints' })} />)}
        </Form.Item>
      </Form>
    </Modal>
  );
